...
# (b) README에 근거가 적힌 불변식을 기계로 지킨다. 전부 컴파일러가 잡아주지 않는 것들이다.
printf 'lint  ... '
...
# Apache POI는 의도적으로 넣지 않는다 (~7MB + IDE 클래스로더 충돌). README '설계 메모' 참고.
...
for f in $(grep -rl 'XMLInputFactory\.newInstance()' src/main/kotlin 2>/dev/null); do
...
grep -rn 'printStackTrace()' src/main/kotlin 2>/dev/null | head -5 | while IFS= read -r hit; do
  printf '  - 스택트레이스 출력: %s\n' "$hit"
done > /tmp/sheetview-lint-trace.$$
if [ -s /tmp/sheetview-lint-trace.$$ ]; then
  violations="$violations
$(cat /tmp/sheetview-lint-trace.$$)"
fi
rm -f /tmp/sheetview-lint-trace.$$
